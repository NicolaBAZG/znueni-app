...
// POST delete winner
app.post('/api/delete-winner', async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE state SET claimed = claimed - 1 WHERE id = 1 AND claimed > 0');
    const stateRes = await pool.query('SELECT claimed FROM state WHERE id = 1');
    const winnersRes = await pool.query('SELECT id, name, TO_CHAR(claimed_at, \'HH24:MI\') as time FROM winners ORDER BY id');
    res.json({ success: true, claimed: stateRes.rows[0].claimed, max: MAX_ZNUENI, winners: winnersRes.rows });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'db_error' });
  }
